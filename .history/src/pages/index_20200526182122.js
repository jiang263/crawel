    ...
    fetch(`${SERVER_URL}/mz/fetch`, {
      ...
      if (res.state) {
        notification.success({
          message: '抓取完成',
          description: `已成功爬取${res.imgCount}张图片`
        })
        if (res.data) {
          // 求出数据增量
    ...
    <div className={styles.normal}>
      <Spin tip="正在疯狂爬取中..." spinning={isLoading}>
        <>
          ...
          <Select defaultValue={0} style={{ width: '15%', marginRight: '20px' }} placeholder="选择爬取方式" onChange={handleChangeType}>
            <Option value={12}>爬取全部页面</Option>
            <Option value={0}>只爬取当前地址</Option>
            <Option value={1}>爬取2页</Option>
            <Option value={5}>爬取5页</Option>
            <Option value={10}>爬取10页</Option>
          </Select>
              ...
              fetchData.length ? fetchData.map(item => {
                return <div className={styles.item} key={item}>
                  ...
                  <div className={styles.viewLink}>
                    <a href={`${SERVER_URL}/${item}.json`} download={`${item}.json`} target="_blank">下载全部图片</a>
