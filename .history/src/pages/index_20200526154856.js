  ...
  return (
    <div className={styles.normal}>
      <Spin tip="正在疯狂爬取中..." spinning={isLoading}>
        <>
          <Select mode="tags" style={{ width: '50%', marginRight: '20px' }} placeholder="输入爬取地址" onChange={handleChange.bind('data')}>
            {list.map(item => {
              return <Option key={item}>{item}</Option>
            })}
          </Select>
          <Select placeholder="选择爬取方式" onChange={handleChangeType.bind('selected')}>
