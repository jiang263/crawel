  ...
  Apify.main(async () => {
    ...
    async function crawler(browser, ourDomain) {
      ...
      const handlePageFunction = async ({ request, $ }) => {
        ...
        const imsgs = $(".col img");
        Array.from(imsgs).forEach(async el => {
          let src = $(el).attr('src');
          const viewSource = await page.goto(ourDomain + src, { waitUntil: 'domcontentloaded' });
          const buffer = await viewSource.buffer()
          console.log(ourDomain + src);
          await fs.writeFileSync(`./static/${path.basename(src)}.${path.extname(src)}`,)
        })
        const enqueued = await enqueueLinks({
          $,
          requestQueue,
          pseudoUrls: [`${ourDomain}/[.*]`],
