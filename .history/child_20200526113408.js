...
function startCrawel(urls, cb) {
  Apify.main(async () => {
    ...
    async function splider(sources) {
      const requestQueue = await Apify.openRequestQueue();
      // await requestQueue.addRequest({ url: 'https://www.baidu.com/' });
      ...
      const handlePageFunction: Apify.CheerioHandlePage = async ({ request, $ }) => {
          ...
          //     .map((i: number, el: HTMLElement) => $(el).attr('href'))
          //     .get();
          ...
          // console.log(`${sameDomainLinks.length} URLS`);
          const imsgs: HTMLImageElement[] = $(".col img");
          const queue:Promise<number>[] = [];
          // Array.from(imsgs).forEach(el => {
          //     let src = $(el).attr('src');
          //     console.log(`${ourDomain}${src}`)
          //     queue.push(fetchData(src))
          // })
          // await Promise.all(queue)
          const enqueued = await enqueueLinks({
              $,
              requestQueue,
              pseudoUrls: ['http[s?]://www.4kbizhi.com/[.*_.*]'],
          ...
          console.log(`${request.url}标题是：${title}`);
