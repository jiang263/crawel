...
function startCrawel(urls, cb) {
  Apify.main(async () => {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      queue.push(crawler(browser, urls[i]))
      if (i &&
        (i + 1) % max_parallel === 0
        || i === (urls.length - 1)) {
    ...
    async function crawler(browser, ourDomain) {
      ...
      const handlePageFunction = async ({ request, page }) => {
        ...
        for (let index = 0; index < urls.length; index++) {
          ...
          await fs.writeFileSync(`./static/${path.basen
        }
        ...
        // const pages = await browser.newPage();
        // const viewSource = await pages.goto(urls[0]);
        // const buffer = await viewSource.buffer()
        // await fs.writeFileSync(`./static/${path.basename(urls[0])}.${path.extname(urls[0])}`, buffer)
        // console.log(`${path.basename(urls[0])}下载完成`);
